...
% Load your complex MR data
data = truth .* mask;  % Apply mask to the ground truth data
...
for i = 1:length(SNRdB)
    ...
    I_noise = add_awgn_noise(truth, SNRdB(i));
    ...
    % Define ROIs 
    N = floor(6/2);  % Half-width of the ROI
    size_im = size(recon_image);  % Use the noisy image size
    x_len = size_im(1);
    y_len = size_im(2);
    ...
    % Using a corner or an edge to capture noise (top-left corner in this example)
    noiseROI = recon_image(1:2*N, 1:2*N);   % Adjust the indices based on your image size
    ...
    annotation('textbox', [0, 0.9 - (i - 1) * 0.1, 1, 0.1], ...
...
function [y, noise] = add_awgn_noise(x, SNR_dB)
    % Calculate signal power
    signal_avg = mean(x);
    SNR = 10 ^ (SNR_dB / 10);
    
    % Calculate noise power in dB
    noise_std = signal_avg / SNR;
    
    % Generate Gaussian noise with appropriate power
    noise = (noise_std / 2) .* (randn(size(x)) + 1j * randn(size(x)));  % Complex noise
